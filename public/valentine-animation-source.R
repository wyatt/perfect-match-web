...
# animated plot
saveGIF({
  fill_steps <- 40  # heart fill-in frames
  ...
  for (i in seq(fill_steps + float_steps)) {
    ...
    p <- ggplot(heart_full[seq(num_hearts), ], 
      ...
      labs(x = "Am I your Perfect Match?", y = "", color = "", size = "")+
      theme(axis.title.x = element_text(color = "#6b7280", size = 28),
            axis.title.y = element_text(color = "#6b7280", size = 28))
    print(p) 
  }
}, 
